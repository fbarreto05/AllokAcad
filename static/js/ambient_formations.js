document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDeleteConfirmation() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                const formationName = this.closest('.formation-item')
                    ?.querySelector('.formation-name')?.textContent || 'esta formação';
                
                const deleteUrl = this.href;
                
                showConfirmModal(
                    'Confirmar Exclusão',
                    `Tem certeza que deseja excluir a formação "${formationName}"?`,
                    'Esta ação não pode ser desfeita. Todos os dados relacionados a esta formação serão permanentemente removidos.',
                    'error',
                    () => {
    ...
    if (!document.querySelector('#ambient-formations-styles')) {
        const styles = document.createElement('style');
        styles.id = 'ambient-formations-styles';
        styles.textContent = `
            @keyframes fadeIn {
                from {
                    opacity: 0;
