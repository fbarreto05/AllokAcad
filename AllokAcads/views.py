    ...
    if name or classroom_ids or professor_ids or formation_ids:
        ...
        if classroom_ids:
            ...
            for classroom_id in classroom_ids:
                ...
                classroom_preference = Classroom_Preference(classroom=classroom, classroom_weight=classroom_weight)
                classroom_preference.save()
                subject.ideal_class.add(classroom_preference)
...
def ambient_delete_subjects(request, subjectid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
    subject = Subject.objects.get(id = subjectid)
    subject.delete()
...
def ambient_delete_classes(request, classid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
    tclass = Class.objects.get(id = classid)
    tclass.delete()
...
def ambient_edit_rooms_validate(request, roomid, ambientid, userid):
    ...
    if name or capacity or roomtype:
        room.save()
    return redirect(f'/AllokAcad/ambient/resources/rooms/{ambient.ambientid}/{user.userid}')
def ambient_delete_rooms(request, roomid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
    room = Classroom.objects.get(id = roomid)
    room.delete()
...
def ambient_edit_formations(request, formationid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
...
def ambient_edit_formations_validate(request, formationid, ambientid, userid):
    ...
    return redirect(f'/AllokAcad/ambient/resources/formations/{ambient.ambientid}/{user.userid}')
def ambient_delete_formations(request, formationid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
    formation = Formation.objects.get(id = formationid)
    formation.delete()
...
def ambient_edit_roomtypes_validate(request, roomid, ambientid, userid):
    ...
    if name:
        roomtype.name = name
        roomtype.save()
    return redirect(f'/AllokAcad/ambient/resources/roomtypes/{ambient.ambientid}/{user.userid}')
def ambient_delete_roomtypes(request, roomid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
    roomtype = ClassroomTP.objects.get(id = roomid)
    roomtype.delete()
...
def ambient_edit_admtypes_validate(request, admtypeid, ambientid, userid):
    ...
    return redirect(f'/AllokAcad/ambient/resources/admtypes/{ambient.ambientid}/{user.userid}')
def ambient_delete_admtypes(request, admtypeid, ambientid, userid):
    ambient = Ambient.objects.get(ambientid = ambientid)
    user = User.objects.get(userid = userid)
...
def ambient_create_admtypes_validate(request, ambientid, userid):
    ...
    return redirect(f'/AllokAcad/ambient/resources/admtypes/{ambient[0].ambientid}/{user[0].userid}')
        ...
        for not_atribuited_activitie in not_atribuited_activities:
            ...
            for candidate in candidates:
                ...
                if not_atribuited_activitie.tclass.favorite_professors.all().filter(professor = candidate):
                    class_preference = not_atribuited_activitie.tclass.favorite_professors.all().get(professor = candidate).professor_weight
                if candidate.num_uses + not_atribuited_activitie.tclass.necessary_subjects.get(subject = not_atribuited_activitie.tsubject).periods <= ambient.max_actv_in_cicle:
                    if subject_preference != 0 and class_preference != 0 and subject_preference + class_preference > highest_weight:
