...
def ambient_solicitations(request, ambientid):
    if request.user.is_authenticated:
        ...
        if ambient.members.filter(user = user) and member[0].admin_type.can_gerenciate_members:
            ...
            return render(request, "AllokAcads/ambient_solicitations.html", {'solicitations' : solicitations, 'ambient' : ambient, 'user' : user, 'userid': userid})
        else:
            return redirect('home')
    else:
        return redirect('login')
...
def ambient_profile_edit_validate(request, ambientid):
    if request.user.is_authenticated:
        ...
        if ambient[0].members.filter(user = user):
            ...
            member.save()
            return render(request, "AllokAcads/ambient_profile_edit.html", {'ambient': ambient[0], 'user': user, 'userid': userid})
        else:
            return redirect('home')
    else:
        return redirect('login')
