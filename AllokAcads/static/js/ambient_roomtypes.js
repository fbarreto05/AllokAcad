document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDeleteConfirmation() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                const roomtypeName = this.closest('.roomtype-card')
                    ?.querySelector('.roomtype-name')?.textContent || 'este tipo de sala';
                
                const deleteUrl = this.href;
                
                showConfirmModal(
                    'Confirmar Exclusão',
                    `Tem certeza que deseja excluir o tipo de sala "${roomtypeName}"?`,
                    'Esta ação não pode ser desfeita. Todos os dados relacionados a este tipo de sala serão permanentemente removidos.',
                    'error',
                    () => {
    ...
    if (!document.querySelector('#ambient-roomtypes-styles')) {
        const styles = document.createElement('style');
        styles.id = 'ambient-roomtypes-styles';
        styles.textContent = `
            @keyframes fadeIn {
                from {
                    opacity: 0;
