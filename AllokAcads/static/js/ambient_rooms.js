document.addEventListener('DOMContentLoaded', function() {
    ...
    function setupDeleteConfirmation() {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                ...
                const roomName = this.closest('.room-card')
                    ?.querySelector('.room-name')?.textContent || 'esta sala';
                
                const deleteUrl = this.href;
                
                showConfirmModal(
                    'Confirmar Exclusão',
                    `Tem certeza que deseja excluir a sala "${roomName}"?`,
                    'Esta ação não pode ser desfeita. Todos os dados relacionados a esta sala serão permanentemente removidos.',
                    'error',
                    () => {
    ...
    if (!document.querySelector('#ambient-rooms-styles')) {
        const styles = document.createElement('style');
        styles.id = 'ambient-rooms-styles';
        styles.textContent = `
            @keyframes fadeIn {
                from {
                    opacity: 0;
